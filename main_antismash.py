...
def downloadProtein(dataframe, work_dir):
    ...
    seq_list = []
    content_list = []
    for tmp_id in dataframe.loc[:,'Entry'].to_list():
        if tmp_id not in id_list:
            id_list.append(tmp_id)
            try:
                tmp_seq = dataframe.loc[dataframe['Entry'] == tmp_id,'Sequence'].to_list()[0]
            except KeyError:
                tmp_seq = dataframe.loc[dataframe['Entry'] == tmp_id,'Sequence'].to_list()
            except AttributeError:
                tmp_seq = dataframe.loc[dataframe['Entry'] == tmp_id,'Sequence']
...
def runAntismash(query_string, work_dir, space_len,neighbour, n_threads):
    ...
    ID_list = []
    candidate_gbk_file_list =  glob.glob(os.path.join(work_dir,'Candidate_genomes_new','*.gbk'))
    for candidate_gbk_file in candidate_gbk_file_list:
        ...
        os.makedirs(os.path.join(work_dir, 'Anitismash_Result_new', refseq_accession), exist_ok=True)
        ID_list.append(ID)
    ...
    for ID, input_file in input:
        refseq_accession = 'GCF_'+str(ID)
        command = ' '.join(['antismash', '--hmmdetection-strictness','strict' ,  '-c', '1', '--taxon', 'bacteria', input_file , '--output-dir', os.path.join(work_dir, 'Anitismash_Result_new', refseq_accession)])
...
combined_df = pd.read_csv(work_dir + 'Query_result.xls', sep ='\t')
merged_df = pd.read_csv(work_dir + 'Filtered_Query_result.xls', sep ='\t')
...
# Anitismash_Result_path = os.path.join(work_dir, 'Anitismash_Result_new')
